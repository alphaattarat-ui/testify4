...
// Define the type for our knowledge base object
...
export const AvatarConfig: React.FC<AvatarConfigProps> = ({
  ...
  // --- Using hardcoded knowledge bases as a workaround ---
  const knowledgeBases = KNOWLEDGE_BASES;
  ...
  return (
    <div className="relative flex flex-col gap-4 w-[550px] py-8 max-h-full overflow-y-auto px-4">
      {/* --- UPDATED: Now uses a dropdown to select a Knowledge Base --- */}
      <Field label="Knowledge Base">
        <Select
          ...
          renderOption={(option: KnowledgeBase) => option.name}
          value={config.knowledgeId}
          onSelect={(option: KnowledgeBase) => onChange("knowledgeId", option.id)}
          // --- MISSING PROP ADDED ---
          isSelected={(option: KnowledgeBase) => option.id === config.knowledgeId}
          // --- END MISSING PROP ---
        />
      </Field>
